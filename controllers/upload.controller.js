...
export async function uploadProfil(req, res) {
    ...
    try {
        // Write the file buffer directly
        await fs.writeFile(uploadPath, req.file.buffer);

        // Update the user model with the new picture path
        const docs = await UserModel.findByIdAndUpdate(
            req.body.userId,
            { $set: { picture: "./uploads/profil/" + fileName } },
            { new: true, upsert: true, setDefaultsOnInsert: true }
        ...
        res.status(200).json({ message: "File uploaded and user updated successfully!", docs });
    } catch (error) {
        console.error("Error:", error); // Detailed error logging
        res.status(500).json({ message: "File upload or user update failed!", error });
    }
}
